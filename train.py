...
##GPU- num_workers  = 4     # CPU was 2 — GPU needs more workers to keep it fed
...
def denormalise_coordinate(normalised_value, coord_name):
    ...
    return lo + (normalised_value + 1.0) / 2.0 * (hi - lo)
...
def build_target_heatmap(
    ...
    blob_sigma_mm = 6.0,          # sigma in world mm — resolution independent
):
    ...
    Returns
    -------
    np.ndarray (Z, H, W) — multi-source probability volume
    """
    # Convert mm sigma to pixels
    mm_per_pixel_x = (x_range[1] - x_range[0]) / heatmap_size
    mm_per_pixel_y = (y_range[1] - y_range[0]) / heatmap_size
    sigma_px_x = blob_sigma_mm / mm_per_pixel_x
    sigma_px_y = blob_sigma_mm / mm_per_pixel_y

    z_bins  = config.HEATMAP_Z_BINS
    z_range = config.HEATMAP_Z_RANGE

    heatmap = np.zeros((z_bins, heatmap_size, heatmap_size), dtype=np.float32)
    y_grid, x_grid = np.ogrid[:heatmap_size, :heatmap_size]
    z_grid = np.arange(z_bins, dtype=np.float32)

    for sx, sy, sz in sources_mm:
        ix = int(np.clip(
            (sx - x_range[0]) / (x_range[1] - x_range[0]) * heatmap_size,
            0, heatmap_size - 1
        ))
        iy = int(np.clip(
            (sy - y_range[0]) / (y_range[1] - y_range[0]) * heatmap_size,
            0, heatmap_size - 1
        ))

        iz = int(np.clip(
            (sz - z_range[0]) / (z_range[1] - z_range[0]) * z_bins,
            0, z_bins - 1
        ))

        # Anisotropic Gaussian in case x and y pixel scales differ
        dist_sq = ((x_grid - ix) / sigma_px_x)**2 + ((y_grid - iy) / sigma_px_y)**2
        g_xy = np.exp(-0.5 * dist_sq)  # (H, W)
        g_z = np.exp(-((z_grid - iz) ** 2) / (2.0 * (sigma_px_x ** 2)))  # (Z,)
        heatmap += g_z[:, np.newaxis, np.newaxis] * g_xy[np.newaxis, :, :]
...
class ComptonEventDataset(Dataset):
    ...
    # ENABLED: Geometric Data Augmentation (X-flip and Y-flip) for training stability
    """
    ...
    def __getitem__(self, idx):
        ...
        # ════════════════════════════════════════════════════════════════
        # ENABLED: Geometric Data Augmentation (X-flip and Y-flip)
        # Applies random flips to exploit detector symmetry and 4× dataset size
        # ════════════════════════════════════════════════════════════════
        if self.augment:
            # X-flip: negate x-coordinates in features and targets
            if np.random.rand() < 0.5:
                x_feature_cols = [0, 3, 8]  # scatter_x, absorb_x, cone_axis_x
                padded[:n_kept, x_feature_cols] *= -1
            ...
            if np.random.rand() < 0.5:
                ...
                padded[:n_kept, y_feature_cols] *= -1
    ...
    def _empty_sample(self):
        return {
            ...
            "target_heatmap":    torch.zeros(config.HEATMAP_Z_BINS, config.HEATMAP_SIZE, config.HEATMAP_SIZE),
...
def compute_minimum_cost_matching(pred_coords, target_coords, source_valid_mask):
    ...
    matched_pairs : list of lists - per batch (pred_idx, true_idx) assignment pairs
    """
    try:
        ...
        if has_scipy:
            ...
            for i in order:
                ...
                if d[best_j] < float('inf'):
                    ...
                    pairs.append((best_j, int(valid_indices[i_idx])))
    ...
    return (
        matched_pred_indices,
        torch.tensor(count_targets, dtype=torch.long, device=pred_coords.device),
        matched_pairs,
    )
...
def build_confidence_labels_from_matching(matched_pred_indices, batch_size):
    ...
    # ENABLED: Soft labels (0.9/0.1) to reduce overconfidence
    ...
    for b, matched_slots in enumerate(matched_pred_indices):
        ...
        # Original code with hard labels (working version):
        # ════════════════════════════════════════════════════════════
        for slot_idx in range(MAX_SOURCES):
            if slot_idx in matched_slots:
                confidence_labels[b, slot_idx] = 0.9
            else:
                confidence_labels[b, slot_idx] = 0.1
        
        # ════════════════════════════════════════════════════════════
        # Legacy hard labels (disabled)
        # To use: replace soft labels above with 1.0/0.0
        # ════════════════════════════════════════════════════════════
        # for slot_idx in range(MAX_SOURCES):
        #     if slot_idx in matched_slots:
        #         confidence_labels[b, slot_idx] = 0.9  # soft positive
        #     else:
        #         confidence_labels[b, slot_idx] = 0.1  # soft negative
...
def run_one_epoch(model, loader, criterion, optimizer, device, is_training):
    ...
    all_err_x, all_err_y, all_err_z, all_err_xyz = [], [], [], []
    count_correct = 0.0
    n_batches   = 0
    ...
    with ctx:
        for batch in loader:
            ...
            matched_indices, derived_count_targets, matched_pairs = compute_minimum_cost_matching(
            ...
            # Align target coordinates to matched prediction slots for coord loss
            aligned_t_coords = torch.full_like(t_coords, float("nan"))
            for b, pairs in enumerate(matched_pairs):
                for pred_idx, true_idx in pairs:
                    aligned_t_coords[b, pred_idx] = t_coords[b, true_idx]
            ...
            total, lh, lc, lconf, lcount = criterion(
                ...
                pred_coords_and_conf, aligned_t_coords,
            ...
            all_err_x.append(errs["err_x_mm"])
            all_err_y.append(errs["err_y_mm"])
            all_err_z.append(errs["err_z_mm"])
...
class Logger:
    ...
    def log(self, epoch, tr, va, lr, t):
        self.rows.append({
            ...
            "train_err_x_mm": round(tr.get("err_x_mm", 0.0), 4),
            "train_err_y_mm": round(tr.get("err_y_mm", 0.0), 4),
            "train_err_z_mm": round(tr.get("err_z_mm", 0.0), 4),
            ...
            "val_err_x_mm": round(va.get("err_x_mm", 0.0), 4),
            "val_err_y_mm": round(va.get("err_y_mm", 0.0), 4),
            "val_err_z_mm": round(va.get("err_z_mm", 0.0), 4),
    ...
    def print(self, epoch, n_ep, tr, va, lr, t, best):
        ...
        print(f"  Epoch {epoch:4d}/{n_ep}"
              ...
              f" (x:{va.get('err_x_mm', 0.0):.1f} y:{va.get('err_y_mm', 0.0):.1f} z:{va.get('err_z_mm', 0.0):.1f})"
              f"  | LR: {lr:.2e}"
              f"  | {t:5.1f}s{marker}")
...
def train(args):
    ...
    train_csvs = [csv_files[i] for i in range(n_train)]
    val_csvs = [csv_files[i] for i in range(n_train, n_total)]
    
    train_ds = ComptonEventDataset(train_csvs, augment=True)
    val_ds = ComptonEventDataset(val_csvs, augment=False)
    
    print(f"  Train: {n_train} (with augmentation) |  Val: {n_val} (no augmentation)\n")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True,
                              num_workers=4, pin_memory=(device.type=="xpu"),
...
# ← Initialized weights immediately overwritten!
