...
#              │  Encoder          │   Events attend to each other — the
#              │                   │   network learns which events are signal
#              │                   │   vs background (fake coincidences)
#              └─────────┬─────────┘
#                        │  (N_events × D_MODEL)
#              ┌─────────▼─────────┐
#              │  Global Average   │   Aggregate all event representations
#              │  Pool             │   into a single scene-level latent vector
#              └────────┬┬─────────┘
#                       ││  (D_MODEL,)
#              ┌────────┘└───────────────────────┐
#              │                                 │
#   ┌──────────▼───────────┐         ┌────────────▼────────────┐
#   │  CNN Spatial Decoder │         │  Coordinate MLP Head    │
#   │                      │         │                         │
#   │  Reshape latent →    │         │  Linear → ReLU →        │
#   │  (D_MODEL, 4, 4)     │         │  Linear → ReLU →        │
#   │  ConvTranspose ×4 →  │         │  Linear → (3,)          │
#   │  (1, 64, 64)         │         │                         │
#   │  Sigmoid             │         │  Predicts (x, y, z)     │
#   │                      │         │  in normalised [-1,+1]  │
#   │  Spatial heatmap     │         │  space                  │
#   └──────────────────────┘         └─────────────────────────┘
#
# TWO-HEAD DESIGN RATIONALE:
#   The CNN head produces a 2D spatial heatmap — a visual image showing
#   WHERE the source is in XY. This is what the user sees and what makes
#   the result interpretable as a medical image.
#
#   The MLP coordinate head predicts (x, y, z) directly as numbers.
#   This gives exact coordinates for error metric computation (mm accuracy).
#   It also helps the network converge faster because the MSE gradient on
#   coordinates is a direct, strong signal.
#
#   Both heads are trained simultaneously with a shared Transformer encoder.
#   The encoder learns a representation that is useful for BOTH tasks.
#
...
class SpatialHeatmapDecoder(nn.Module):
    ...
    def forward(self, latent_vector: torch.Tensor) -> torch.Tensor:
        ...
        Returns
        -------
        torch.Tensor  shape (batch, 1, heatmap_size, heatmap_size)
...
class CoordinateMlpHead(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float, n_outputs: int = 3):
        ...
        self.mlp = nn.Sequential(
            ...
            # Output layer: predict (x, y, z) in normalised space
            nn.Linear(d_model // 2, n_outputs),
            nn.Tanh(),   # maps to [-1, +1], matching target normalisation
        )
    ...
    def forward(self, latent_vector: torch.Tensor) -> torch.Tensor:
        ...
        torch.Tensor  shape (batch, 3)
            Predicted (x, y, z) in normalised [-1, +1] space.
...
class CountHead(nn.Module):
    ...
    def __init__(self, d_model: int, max_sources: int = 5):
        ...
        # Pin final layer to near-zero weights so logits start uniform
        # This makes cross_entropy ≈ log(6) = 1.79 reliably every run
    ...
    def forward(self, latent_vector: torch.Tensor) -> torch.Tensor:
        ...
        Returns
        -------
        torch.Tensor  shape (batch, max_sources + 1)
            Raw logits for each source count class (0 to max_sources).
        """
...
class ComptonSourceLocaliser(nn.Module):
    ...
    def _initialise_weights(self):
        ...
        EXCLUSION: Count head final layer is already initialized in CountHead.__init__
        with near-zero weights for stable cross-entropy start.
        """
    ...
    def forward(
        ...
        Returns
        -------
        heatmap : torch.Tensor  shape (batch, 1, HEATMAP_SIZE, HEATMAP_SIZE)
...
class ComptonLocalisationLoss(nn.Module):
    ...
    def forward(
        ...
        valid_mask = ~torch.isnan(target_coords[:, :, 0])  # (batch, MAX_SOURCES)
        if valid_mask.any():
            ...
            loss_coord = ((pred_valid - target_valid)**2).sum(dim=-1)  # (batch, MAX_SOURCES)
        ...
        pred_conf_sigmoid = torch.sigmoid(pred_conf_logits)
        
        # Clamp predictions to avoid log(0)
        pred_conf_sigmoid = pred_conf_sigmoid.clamp(1e-7, 1 - 1e-7)
        loss_confidence = -(
            confidence_labels * torch.log(pred_conf_sigmoid) +
            (1 - confidence_labels) * torch.log(1 - pred_conf_sigmoid)
        ).mean()
...
if __name__ == "__main__":
    ...
    dummy_target_heatmap = torch.zeros(batch_size, 1,
                                        config.HEATMAP_SIZE, config.HEATMAP_SIZE)
    dummy_target_heatmap[:, :, 32, 32] = 1.0   # source at centre
