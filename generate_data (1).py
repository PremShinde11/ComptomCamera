...
#   4. Type-2/3 singles driven by real energy distributions
...
def build_scene(sources, n_events, start_id):
    ...
    # ── ADVANCED EVENTS — comment out this block to disable ──────────────
    for _ in range(n2):
        src = sources[np.random.randint(len(sources))]
...
def generate_dataset(n_files, n_events, out_dir, seed_offset, label):
    ...
    for idx in range(n_files):
        ...
        n_src   = int(np.random.randint(1, 6))
        ...
        if (idx + 1) % max(1, n_files // 10) == 0 or idx == n_files - 1:
            ...
            print(f"  [{idx+1:04d}/{n_files}]  {len(df)} events  types={counts}")
