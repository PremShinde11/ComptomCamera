...
def _sync_config_from_checkpoint(ck):
    state = ck.get("model_state", {})

    # Infer D_MODEL and feedforward size from saved weights.
    proj_w = state.get("event_encoder.projection.0.weight")
    if proj_w is not None:
        config.D_MODEL = int(proj_w.shape[0])

    ff_w = state.get("transformer_encoder.layers.0.linear1.weight")
    if ff_w is not None:
        config.DIM_FEEDFORWARD = int(ff_w.shape[0])

    # Infer heatmap channel count (Z bins) from final conv.
    hm_w = state.get("heatmap_decoder.final_conv.0.weight")
    if hm_w is not None:
        config.HEATMAP_Z_BINS = int(hm_w.shape[0])

    # Optional metadata saved in checkpoints.
    if "n_features" in ck:
        config.N_INPUT_FEATURES = int(ck["n_features"])
    if "max_sources" in ck:
        config.MAX_SOURCES = int(ck["max_sources"])
        globals()["MAX_SOURCES"] = config.MAX_SOURCES

    if config.D_MODEL % config.N_HEADS != 0:
        print(f"  [WARN] D_MODEL={config.D_MODEL} not divisible by N_HEADS={config.N_HEADS}.")
...
def load_model(model_path, device):
    ...
    _sync_config_from_checkpoint(ck)
    model = ComptonSourceLocaliser().to(device)
    try:
        model.load_state_dict(ck["model_state"])
    except RuntimeError as e:
        print(f"  [WARN] {e}")
        print("  [WARN] Retrying with strict=False to load matching layers only.")
...
def extract_heatmap_peaks(heatmap_np, depth_unused=None,
    ...
    if heatmap_np.ndim == 3:
        # Z-sliced heatmap volume -> XY max-projection for peak picking
...
def predict_scene(model, sample, device):
    ...
    pred_heatmap    : np.ndarray (HEATMAP_Z_BINS, H, W)
    ...
    heatmap = pred_heatmap_t.squeeze().cpu().numpy()     # (Z, H, W)
...
def parse_args():
    ...
    p.add_argument("--model-path", type=str,
                   default=os.path.join(config.MODEL_DIR, "best_modelV_g.pth"))
    return p.parse_args()
